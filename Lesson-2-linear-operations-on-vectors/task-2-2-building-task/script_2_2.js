...
const createTask=()=>{
    variant = randomInt(0, data.length-1);
    console.log(variant)
    condition.innerHTML = `<div class="hint"><h2>Задание 2.2. Линейные операции над векторами</h2><img class="sistem-hint" src="./../../paint/hint.png"/><img class="teory-hint" src="./../../paint/lamp.png"/></div><p>Последовательно отложите следующие векторы, начиная от конца вектора c:</p>`;
...
btnCheck.addEventListener("click", () => {
    ...
    spanResult.textContent = "Количество баллов: " + mark + " из " + data[variant].coordTaskVectors.length;
    console.log(data, count, coordUserVect, arrUserVector);
    localStorage.setItem("task-5", [mark, checkHint]);
})
...
const checkStorage = () =>{
    if (localStorage.getItem("task-5")){
        if (localStorage.getItem("task-5")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-5")[0];
        if (localStorage.getItem("task-5")[0] === "0") spanResult.textContent = "Количество баллов: 0";  
    }else 
        spanResult.textContent = "Количество баллов: - ";
}
...
const chooseHint = (str)=>{
    ...
    if (str.innerText.indexOf('0') !== -1) return hints[1]
}
