...
const createTaskVect=()=>{
    ...
    condition.innerHTML = `<div class="hint"><h2>Задание 2.1. Сумма и разность векторов</h2><img class="sistem-hint" src="./../../paint/hint.png"/><img class="teory-hint" src="./../../paint/lamp.png"/></div><p>Постройте ${task[variant]}</p><section class="section-progress"><progress value="0" max="10"></progress><p>Задание №${progressValue + 1} / ${data.numberOfTask}</section>`;
...
const checkStorage = () =>{
    if (localStorage.getItem("task-4")){
        if (localStorage.getItem("task-4")[0]) spanResult.textContent = "Результат: " + localStorage.getItem("task-4")[0] + " из 10";
        if (localStorage.getItem("task-4")[0] === "0") spanResult.textContent = "Результат: 0 из 10";  
    }else 
