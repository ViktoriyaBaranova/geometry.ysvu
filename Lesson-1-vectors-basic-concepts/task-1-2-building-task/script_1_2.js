...
const checkStorage = () =>{
    if (localStorage.getItem("task-1")){
        if (localStorage.getItem("task-1")[0]) spanResult.textContent = "Результат: " + localStorage.getItem("task-1")[0] + " из 10 ";
        if (localStorage.getItem("task-1")[0] === "0") spanResult.textContent = "Результат: 0 из 10";  
    }else 
