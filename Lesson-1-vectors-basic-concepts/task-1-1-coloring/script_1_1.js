...
btnCheck.addEventListener("click", () => {
	...
	if (elem === data[variant].colors[index]){
           count += 1;
           console.log("yes");
        } else {
           count += -1;
...
const checkStorage = () =>{
    if (localStorage.getItem("task-0")){
        if (localStorage.getItem("task-0")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-0")[0] + " из 10";
        if (localStorage.getItem("task-0")[0] === "0") spanResult.textContent = "Количество баллов: 0 из 10";  
    }else 
