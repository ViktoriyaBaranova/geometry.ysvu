...
localStorage.setItem("task-17", "0");
...
let colorProj = data[variant].colorVect; // = randomize(data[variant].colorVect);
colorInput = data[variant].colorVect;
...
const drawTask = (colorVect, colorProj)=>{
    ...
    form.innerHTML = `<p>Скалярная проекция векторов равна:</p>` ;   
    for (let i=0; i<data[variant].colorProj.length; i++){
         form.innerHTML += `<div class="task"><div class="color" style="background-color: ${colorInput[i]};"></div><input style='text'></div>`;
    }
    draw_svg_grid();
    ...
    if (!clickedVect.classList.contains("clicked")){
        ...
        if (chooseVectors.length === 2){
            ...
            chooseVectors.forEach((elem)=>{
                ...
                setTimeout(()=>{ 
                   elem.setAttribute('stroke-width', '1.8');
                }, 700);
            });
        }
    }else{
...
const checkStorage = () =>{
    if (localStorage.getItem("task-17")){
        if (localStorage.getItem("task-17")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-17")[0] + " из 12";
        if (localStorage.getItem("task-17")[0] === "0") spanResult.textContent = "Количество баллов: 0 из 12";  
    }else 
        spanResult.textContent = "Количество баллов: - из 12";
...
btnCheck.addEventListener('click', ()=>{
   ...
   for (let i=0; i<data[variant].projections.length; i++){
    ...
    console.log("count vect", data);
    let allInputs = document.querySelectorAll("input");
    allInputs.forEach((el, ind)=>{
       if (el.value == data[variant].projections[ind][5]) countInput++; 
       console.log(el.value, data[variant].projections[ind][5]);
    });
...
imgHint.addEventListener("click", () => {
    ...
    steps: [
        ...
        },
        {
            ...
            intro: "Нажимайте на пары - цветная геометрическая проекция и ее вектор"
        }        
    ]
    }).start();
});
