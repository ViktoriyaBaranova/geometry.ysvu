...
const checkStorage = () =>{
    if (localStorage.getItem("task-19")){
        if (localStorage.getItem("task-19")[0]) spanResult.textContent = "Результат: " + localStorage.getItem("task-19")[0] + " из 10";
        if (localStorage.getItem("task-19")[0] === "0") spanResult.textContent = "Результат: 0 из 10";  
    }else 
...
btnCheck.addEventListener("click", () => {
    ...
    localStorage.setItem("task-19", [mark, checkHint]);
});
