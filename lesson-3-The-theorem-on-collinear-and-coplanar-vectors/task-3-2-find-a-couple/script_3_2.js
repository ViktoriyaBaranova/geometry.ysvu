...
const createArrNum = ()=>{
    ...
    for (let i=0; i<numberOfCards; i++){
        ...
        //console.log([[a1, a2], [a1*ind, a2*ind], ind]);
...
btnCheck.addEventListener('click', ()=>{
    ...
    rightPairs.forEach((card, index) =>{
        if (card[0].getAttribute("alfa") === arrInputs[index].value){
            countRightInput++;
            arrInputs[index].style.background = "lightgreen";
        } else {
         arrInputs[index].style.background = "red";
    }
    });
    ...
    const mark = countRightPair.length/2 + countRightInput; 
...
const checkStorage = () =>{
    if (localStorage.getItem("task-9")){
        if (localStorage.getItem("task-9")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-9")[0] + " из "+ score;
        if (localStorage.getItem("task-9")[0] === "0") spanResult.textContent = "Количество баллов: 0 из "+ score;  
    }else 
        spanResult.textContent = "Количество баллов: - из "+ score;
}
