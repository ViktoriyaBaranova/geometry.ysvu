...
"use strict";
localStorage.setItem("task-10", "0");
...
btnCheck.addEventListener('click', ()=>{
    ...
    localStorage.setItem("task-10", [mark, checkHint]);
});

const checkStorage = () =>{
    if (localStorage.getItem("task-10")){
        if (localStorage.getItem("task-10")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-10")[0] + " из 10";
        if (localStorage.getItem("task-10")[0] === "0") spanResult.textContent = "Количество баллов: 0 из 10";  
    }else 
...
lampHint.addEventListener("click", () => {
    ...
    localStorage.setItem("task-10", [mark, checkHint]);
    if (hint){
        p_task.setAttribute("data-tooltip", "На плоскости любой вектор можно разложить по двум данным неколлинеарным векторам, причем коэффициенты разложения определяются единственным образом.");
...
imgHint.addEventListener("click", () => {
    ...
    steps: [
        ...
        {
            element: document.querySelector("img.face"),
