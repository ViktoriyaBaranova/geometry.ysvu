...
const score = 10;
let  count = 0, mark = 0;
localStorage.setItem("task-13", "0");
...
const checkStorage = () =>{
    if (localStorage.getItem("task-13")){
        if (localStorage.getItem("task-13")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-13")[0] + " из 10";
        if (localStorage.getItem("task-13")[0] === "0") spanResult.textContent = "Количество баллов: 0 из 10";  
    }else 
...
btnCheck.addEventListener("click", () => {
    ...
    coordVectSum.forEach((el, index)=>{
        count += (el.every((val, ind)=>val == userCoordVect[index][ind])) ? 1 : 0;
    });
    ...
    localStorage.setItem("task-13", [mark, checkHint]);
});
...
lampHint.addEventListener("click", () => {
    ...
    localStorage.setItem("task-13", [mark, checkHint]);
    if (hint){
        p_task.setAttribute("data-tooltip", "Равенство = ax + by называется разложением вектора по векторам (a, b); числа x, y называются координатами вектора в системе (a, b)");
