...
let  count = 0, mark = 0;
//coords.basis
localStorage.setItem("task-12", "0");
...
const checkStorage = () =>{
    if (localStorage.getItem("task-12")){
        if (localStorage.getItem("task-12")[0]) spanResult.textContent = "Количество баллов: " + localStorage.getItem("task-12")[0] + " из 12";
        if (localStorage.getItem("task-12")[0] === "0") spanResult.textContent = "Количество баллов: 0 из 12";  
    }else 
...
btnCheck.addEventListener("click", () => {
    ...
    arrVectSum.forEach((el, ind)=>{
        count += (el.every((val, ind)=>val == coordGreyVect[i][ind])) ? 1 : 0;
        i++;
    });
